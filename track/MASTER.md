...
| SRV-006 | verified | P1 | Server | server logger 작성 | API/Run 이벤트 기록용 logger wrapper가 있다 |
| SRV-007 | verified | P1 | Server | server error boundary/not-found 처리 | 기본 에러 UX가 있다 |
...
| DB-006 | verified | P0 | DB | AgentRun 모델 정의 | run 상태와 issue/project 연결이 가능하다 |
...
| DB-011 | blocked | P1 | DB | migration 실행 검증 | local DB에 migrate가 성공한다 |
| DB-012 | verified | P1 | DB | Prisma repository layer 작성 | 주요 모델 CRUD 함수가 lib 계층에 있다 |
...
| API-002 | verified | P0 | API | Project list/create API | 프로젝트 목록/생성이 가능하다 |
| API-003 | verified | P0 | API | Project detail/update API | 프로젝트 조회/수정이 가능하다 |
...
| API-008 | verified | P0 | API | Run create/list API | run 생성/목록 조회가 가능하다 |
...
| MCP-003 | verified | P0 | MCP | `project.list/get` tools | MCP로 프로젝트 조회가 가능하다 |
| MCP-004 | verified | P0 | MCP | `issue.list/get/create` tools | MCP로 이슈 조회/생성이 가능하다 |
| MCP-005 | verified | P0 | MCP | `run.start/status` tools | MCP로 run 시작/상태 조회가 가능하다 |
...
| MCP-012 | implemented | P1 | MCP | MCP smoke test 작성 | tool 호출 smoke test가 통과한다 |
...
| EXT-004 | verified | P0 | Extension | Orchestrator REST client 작성 | serverUrl/token 기반 API 호출이 가능하다 |
...
| EXT-013 | verified | P1 | Extension | Artifact/Diff view command | diff/test artifact를 열 수 있다 |
| EXT-014 | verified | P1 | Extension | notification 구현 | run 완료/승인 요청 알림이 뜬다 |
| EXT-015 | verified | P1 | Extension | refresh/reconnect command | 수동 갱신과 재연결이 가능하다 |
| EXT-016 | verified | P2 | Extension | Webview CSP 정리 | 기본 CSP가 설정된다 |
...
| SDK-002 | verified | P0 | SDK | `CodingRunInput` 타입 정의 | runId/issue/workspace/systemInstruction 등이 포함된다 |
| SDK-003 | verified | P0 | SDK | `CodingAgentEvent` 타입 정의 | message/tool/file/command/test/done 이벤트 타입이 있다 |
| SDK-004 | verified | P0 | SDK | Claude Agent SDK adapter skeleton | start/cancel 메서드 구조가 있다 |
...
| SDK-013 | verified | P1 | SDK | SDK Runner dry-run mode | 실제 수정 없이 prompt/plan 검증이 가능하다 |
| SDK-014 | verified | P1 | SDK | SDK adapter smoke test | fake adapter 또는 mock으로 event stream 테스트가 있다 |
...
| WFL-005 | verified | P0 | Workflow | Artifact 저장 규칙 구현 | plan/diff/test/review artifact가 저장된다 |
| WFL-006 | verified | P1 | Workflow | Risk assessment helper | 변경 파일 기반 risk score가 계산된다 |
| WFL-007 | verified | P1 | Workflow | sensitive file detector | auth/db/deploy 파일 변경을 감지한다 |
| WFL-008 | verified | P1 | Workflow | approval evidence rendering | approval 상세에 근거가 표시된다 |
...
| GIT-007 | verified | P1 | Git | optional Draft PR command | 최종 승인 후에만 Draft PR 생성이 가능하다 |
| GIT-008 | verified | P2 | Git | PR body template | plan/diff/test/approval 링크가 포함된다 |
...
| UI-002 | verified | P1 | UI | Project list page | 프로젝트 목록이 보인다 |
...
| TST-004 | implemented | P0 | Test | MCP smoke 검증 | 핵심 MCP tools 호출이 성공한다 |
| TST-005 | verified | P0 | Test | Extension activation 검증 | extension host에서 activate 된다 |
...
| TST-013 | verified | P1 | Test | 단일 명령 임시 dev stack 작성 | 한 명령으로 PGlite 임시 DB 준비, migrate/seed, 서버 실행이 가능하다 |
